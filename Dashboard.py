...
def det_launch_outcome(launch_outcome):
    if launch_outcome == 1:
        return 'success'
    else:
        return 'failure'
...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records',
                                        style={'textAlign': 'center', 'color': '#507D34',
                                               'font-size': 40}),
                                html.Br(),
...
def get_scatter_plot(entered_site, slider_range_list):
    ...
    title = f'Correlation Payload / Success for {entered_site} '
    if entered_site == 'ALL':
        fig = px.scatter(spacex_df, x='Payload Mass (kg)', y='class', color='Booster Version Category', title='Correlation Payload / Success for all Sites')
        return fig
    else:
